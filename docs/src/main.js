...
if (!React || !ReactDOM || !htm) {
  throw new Error('React/ReactDOM/htm не загрузились из CDN. Проверьте index.html.');
}
...
const api = async (action, payload = null) => {
  if (!APPS_SCRIPT_URL) throw new Error('Вставьте URL в config.js');
  ...
  return html`
    <div style=${wrap}>
      <form style=${card} onSubmit=${async (e) => {
        ...
        <h2>Вход в CRM</h2>
        <p style="margin:0;color:#666;font-size:12px">v3</p>
        <input style=${input} placeholder="Логин" value=${identity} onInput=${(e) => setIdentity(e.target.value)} />
        ...
        ${workspaces.length > 0 ? html`
          <select style=${input} value=${workspaceId} onChange=${(e) => setWorkspaceId(e.target.value)}>
            ${workspaces.map((w) => html`<option value=${w.id}>${w.name}</option>`)}
          </select>
        ` : null}
        ${error ? html`<p style="color:red">${error}</p>` : null}
        <button style=${btnPrimary}>${workspaces.length ? 'Войти' : 'Вход'}</button>
      </form>
    </div>
  `;
}
...
function App() {
  ...
  if (!session?.token) {
    return html`<${LoginPage} onLogin=${login} />`;
  }
  ...
  return html`
    <div style=${appWrap}>
      <header style=${header}>
        ...
        <div>
          <button style=${btn} onClick=${loadAll}>Обновить</button>
          <button style=${btn} onClick=${logout}>Выйти</button>
        </div>
      </header>
      <nav style=${tabs}>
        ${pages.map(([id, label]) => html`
          <button style=${page === id ? btnPrimary : btn} onClick=${() => setPage(id)}>
            ${label}
          </button>
        `)}
      </nav>
      ...
      ${page === 'dashboard' ? html`
        <section style=${grid2}>
          <${Metric} title="Склад" value=${dashboard.active_stock || 0} />
          <${Metric} title="Стоимость" value=${money(dashboard.stock_value)} />
      ...
      ${page === 'inventory' ? html`
        <section>
          ${canEdit ? html`<button style=${btnPrimary} onClick=${addPurchase}>+ Покупка</button>` : null}
          <table style=${table}>
            <thead>
              <tr><th>№</th><th>Модель</th><th>Статус</th><th>Себест</th><th>Продажа</th><th>Действия</th></tr>
            </thead>
            <tbody>
              ${items.map((i) => html`
                <tr>
                  ...
                  <td>${i.model_name}</td>
                  <td>${i.status || '—'}</td>
                  <td>${money(i.total_cost)}</td>
                  <td>${i.sale_price ? money(i.sale_price) : '—'}</td>
                  <td>
                    ${canEdit ? html`
                      <button style=${btn} onClick=${() => sellItem(i)}>Продать</button>
                      ${i.status === 'sold' ? html`
                        <button style=${btnDanger} onClick=${() => cancelSale(i)}>Отмена</button>
                      ` : null}
                    ` : html`<span style="color:#666">read-only</span>`}
                  </td>
                </tr>
              `)}
            </tbody>
      ...
      ${canEdit && page === 'activity' ? html`
        <section>
          <table style=${table}>
            <thead><tr><th>Время</th><th>№</th><th>Действие</th></tr></thead>
            <tbody>
              ${activity.map((a) => html`
                <tr><td>${a.timestamp}</td><td>${a.item_number}</td><td>${a.action}</td></tr>
              `)}
            </tbody>
          </table>
        </section>
...
function Metric({ title, value }) {
  return html`
    <div style=${metric}>
      ...
      <div style="font-size:18px;font-weight:700">${value ?? 0}</div>
    </div>
  `;
}
