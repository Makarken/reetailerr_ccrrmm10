...
function getDashboard() {
  ...
  const monthSales = sales.filter((s) => monthKey(s.sale_date || s.timestamp) === month);
  const paidMonthSales = monthSales.filter((s) => boolText(s.money_received) === 'yes');
  ...
  const paidProfit = paidMonthSales.reduce((a, s) => a + toNum(s.profit || (toNum(s.sale_price) - toNum(s.total_cost))), 0);
  const processingProfit = monthSales.reduce((a, s) => a + (boolText(s.money_received) === 'yes' ? 0 : toNum(s.profit || (toNum(s.sale_price) - toNum(s.total_cost)))), 0);

  return {
    active_stock: active.length,
    stock_value: active.reduce((a, i) => a + toNum(i.total_cost), 0),
    sold_this_month: monthSales.length,
    profit_this_month: paidProfit,
    profit_processing: processingProfit,
    potential_profit_this_month: paidProfit + processingProfit,
    profit_share_each: paidProfit / 3,
...
function updateMoneyReceived(itemNumber, moneyReceived) {
  ...
  const nextMoney = boolText(moneyReceived);
  item.money_received = nextMoney;
  item.updated_at = nowIso();
  saveInventoryItem(item);

  const sales = scopedRows(CONFIG.SHEETS.sales, CONFIG.HEADERS.sales);
  const sale = sales.find((s) => String(s.sale_id) === String(item.sale_id) && String(s.is_cancelled || 'no') !== 'yes')
    || sales.filter((s) => String(s.item_number) === String(item.item_number) && String(s.is_cancelled || 'no') !== 'yes').slice(-1)[0];
  if (sale) {
    sale.money_received = nextMoney;
    saveSale(sale);
  }
