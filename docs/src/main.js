...
function CrmApp({ onLogout, session, onSessionUpdate }) {
  ...
  return html`<div className="min-h-screen pb-24 bg-gradient-to-br from-luxe-bg via-luxe-bg to-[#ece6db]">
    ...
    <nav className="bottom-nav">
      ${BOTTOM_PAGES.map(([id, label, icon]) => html`<button onClick=${() => setPage(id)} className=${`nav-btn tap-btn ${id === page ? 'active' : ''}`}>${icon} ${label}</button>`)}
    </nav>
...
function SaleModal({ close, items, save, initialItemNumber = '' }) {
  ...
  return html`<div className="fixed inset-0 bg-overlay pt-16 px-3 pb-3 md:px-8 md:pb-8 z-30 overflow-auto"><form onSubmit=${(e) => { e.preventDefault(); if (!invalid) save({ ...f, item_number: selectedItem.item_number, sale_date: new Date().toISOString().slice(0, 10) }); }} className="max-w-xl mx-auto premium-card rounded-2xl p-4 space-y-3"><div className="flex justify-between"><h2 className="font-semibold text-lg">Продажа</h2><button type="button" onClick=${close}>✕</button></div>
    ...
    ${selectedItem ? html`<div className="rounded-xl bg-item-warm p-3 text-sm">Товар: <b>№${selectedItem.item_number} · ${selectedItem.model_name || '—'}</b><br/>Себестоимость: <b>${money(selectedItem.total_cost)}</b> · Статус: <b>${STATUS_META[selectedItem.status]?.label || selectedItem.status}</b></div>` : null}
    <div className="grid md:grid-cols-2 gap-2">
