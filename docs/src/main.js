...
const STATUS_META = {
  ...
  japan_transit: { label: 'В пути с Японии', icon: '✈️', cls: 'bg-sky-100 text-sky-700 border-sky-200' },
...
const AnalyticsChart = ({ series = [] }) => {
  ...
  return html`<div className="overflow-x-auto"><svg viewBox=${`0 0 ${width} ${height}`} className="min-w-[640px] w-full h-56">
    ...
    <div className="text-xs text-luxe-muted flex flex-wrap gap-3"><span>● Выручка</span><span className="text-green-700">● Прибыль</span><span className="text-blue-700">● Кол-во</span></div>
  </div>`;
};

function CrmApp({ onLogout, session, onSessionUpdate }) {
  ...
  const analyticsYearSeries = useMemo(() => Object.entries(analytics.monthly || {})
    ...
    .map(([m, v]) => ({ month: m, sold_count: Number(v.sold_count || 0), revenue: Number(v.revenue || 0), profit: Number(v.profit || 0), profit_processing: Number(v.profit_processing || 0), potential_profit: Number(v.potential_profit || 0), avg_markup: Number(v.avg_markup || 0) })), [analytics, analyticsYear]);
  ...
  const deleteItem = async (itemNumber) => {
    if (!await showConfirm(`Удалить карточку товара №${itemNumber} полностью?`)) return;
  ...
  const completeRepair = async (itemNumber, repairCost = null) => {
    ...
    if (value == null) {
      value = await showPrompt('Введите стоимость ремонта (€)', '0');
  ...
  const switchWorkspace = async (workspaceId) => {
    try {
      setError('');
      const r = await api('switchWorkspace', { workspace_id: workspaceId });
      if (r?.workspace_id && onSessionUpdate) {
        onSessionUpdate({ workspace_id: r.workspace_id, workspace_name: r.workspace_name });
      }
  ...
  const handleCreateWorkspace = async () => {
    const name = await showPrompt('Название новой базы');
    if (name == null || !String(name).trim()) return;
    const viewerLogin = await showPrompt('Логин для зрителя новой базы');
    if (viewerLogin == null || !String(viewerLogin).trim()) return;
    const viewerPassword = await showPrompt('Пароль для зрителя новой базы (мин. 3 символа)');
    if (viewerPassword == null || String(viewerPassword).trim().length < 3) return;
    try {
      await api('createWorkspace', { name: String(name).trim(), viewer_login: String(viewerLogin).trim(), viewer_password: String(viewerPassword).trim() });
      setToast('База создана');
      setShowWorkspaceSwitcher(false);
      loadAll();
  ...
  const handleDeleteWorkspace = async (workspaceId, workspaceName) => {
    if (!await showConfirm(`Вы точно хотите удалить базу "${workspaceName}"? Это действие нельзя отменить.`)) return;
    try {
      await api('deleteWorkspace', { workspace_id: workspaceId });
      setToast(`База "${workspaceName}" удалена`);
      setShowWorkspaceSwitcher(false);
      loadAll();
    } catch (e) { setError('Ошибка: ' + e.message); }
  };

  const handleChangeCredentials = async (workspaceId, workspaceName) => {
    const newLogin = await showPrompt(`Новый логин для зрителя базы "${workspaceName}"`);
    if (newLogin == null || !String(newLogin).trim()) return;
    const newPassword = await showPrompt(`Новый пароль для зрителя базы "${workspaceName}" (мин. 3 символа)`);
    if (newPassword == null || String(newPassword).trim().length < 3) return;
    try {
      await api('changeViewerCredentials', { workspace_id: workspaceId, new_login: String(newLogin).trim(), new_password: String(newPassword).trim() });
      setToast('Учётные данные обновлены');
      setShowWorkspaceSwitcher(false);
    } catch (e) { setError('Ошибка: ' + e.message); }
  };
  ...
  return html`<div className="min-h-screen pb-24 bg-gradient-to-br from-luxe-bg via-luxe-bg to-[#ece6db]">
    <header className="sticky top-0 z-20 border-b border-luxe-border bg-luxe-bg/95 backdrop-blur px-4 py-3 flex items-center justify-between flex-wrap gap-2">
      ${!isViewer && html`<button className="tap-btn rounded-xl border border-luxe-border bg-white px-3 py-2 text-sm" onClick=${() => setShowMenu(true)}>☰</button>`}
      ${isViewer && html`<div className="w-10"></div>`}
    ...
    ${showWorkspaceSwitcher && workspaceList.length > 0 ? html`<div className="fixed inset-0 z-50"><div className="absolute inset-0 bg-overlay-light" onClick=${() => setShowWorkspaceSwitcher(false)}></div><div className="absolute top-20 right-4 bg-white rounded-xl shadow-xl p-4 w-72"><h3 className="font-semibold mb-3">Базы данных</h3><div className="space-y-2">${workspaceList.map((w) => html`<div className="flex items-center gap-1"><button className=${`flex-1 text-left rounded-xl px-3 py-2 text-sm tap-btn ${w.id === session?.user?.workspace_id ? 'ws-btn-active' : 'ws-btn-inactive'}`} onClick=${() => switchWorkspace(w.id)}>${w.id === session?.user?.workspace_id ? '✓ ' : ''}${w.name}</button>${isAdmin ? html`<button title="Изменить учётные данные" className="tap-btn rounded-lg border border-luxe-border px-2 py-1 text-xs" onClick=${() => handleChangeCredentials(w.id, w.name)}>🔑</button><button title="Удалить базу" className="tap-btn rounded-lg border border-rose-300 text-rose-600 px-2 py-1 text-xs" onClick=${() => handleDeleteWorkspace(w.id, w.name)}>🗑️</button>` : null}</div>`)}</div>${isAdmin ? html`<button className="tap-btn w-full mt-3 rounded-xl bg-luxe-accent text-white py-2 text-sm" onClick=${handleCreateWorkspace}>+ Создать новую базу</button>` : null}</div></div>` : html``}

    ${showMenu && html`<div className="fixed inset-0 z-40"><div className="absolute inset-0 bg-overlay-light" onClick=${() => setShowMenu(false)}></div><aside className="absolute left-0 top-0 h-full w-72 bg-white shadow-xl p-4"><div className="flex items-center justify-between"><h2 className="font-semibold">Меню</h2><button className="tap-btn" onClick=${() => setShowMenu(false)}>✕</button></div><div className="mt-4 space-y-2">${MENU_PAGES.map(([id, label, icon]) => { if (isViewer && ['analytics', 'qc', 'rephoto'].includes(id)) return null; return html`<button className=${`w-full text-left rounded-xl px-3 py-2 ${page === id ? 'bg-luxe-accent text-white' : 'bg-luxe-bg'}`} onClick=${() => openFromMenu(id)}>${icon} ${label}</button>`; })}</div></aside></div>`}
    ...
    <main className="p-4 max-w-7xl mx-auto space-y-3 fade-in">
      ...
      ${!loading && page === 'dashboard' && html`<section className="space-y-3"><div className="grid grid-cols-2 gap-3">${(isViewer ? [
        ...
        ['💶', 'Прибыль получена', money(dashboard.profit_this_month)],
        ['⏳', 'Прибыль в ожидании', money(dashboard.profit_pending_this_month)],
        ['👥', 'На 1 человека', money(dashboard.profit_share_each)],
        ['🏦', 'Остаток закупа', money(dashboard.purchase_balance || 0)],
        ['📊', 'Средний ROI в месяц', pct(dashboard.avg_monthly_roi || 0)]
      ] : [
        ['📦', 'Активный склад', dashboard.active_stock || 0],
        ['💰', 'Стоимость склада', money(dashboard.stock_value || 0)],
        ['✅', 'Продано в этом месяце', dashboard.sold_this_month || 0],
      ...
      ${!isViewer && html`<div className="premium-card rounded-2xl p-4"><p className="text-sm font-medium">Установить остаток закупа вручную</p><p className="text-xs text-luxe-muted mt-1">Введите точное значение остатка. Введите 0 для автоматического расчёта (сумма себестоимостей проданных позиций с зашедшими деньгами).</p><div className="mt-2 flex gap-2"><input type="number" className="rounded-xl border border-luxe-border p-2 bg-white w-44" value=${purchaseBalanceInput} onInput=${(e) => setPurchaseBalanceInput(e.target.value)}/><button className="tap-btn rounded-xl border border-luxe-border px-3 py-2" onClick=${savePurchaseBalance}>Сохранить</button></div></div>`}
      ...
      ${analyticsType === 'monthly' ? html`<div className="mt-3 space-y-3"><label className="text-xs text-luxe-muted">Месяц<input type="month" className="ml-2 rounded-lg border border-luxe-border p-1 bg-white" value=${salesMonth} onInput=${(e) => setSalesMonth(e.target.value)}/></label><div className="overflow-auto"><table className="min-w-[760px] w-full text-sm"><thead><tr className="text-xs text-luxe-muted"><th className="text-left">№</th><th className="text-left">Модель</th><th>Цена продажи</th><th>Себестоимость</th><th>Прибыль</th><th>В обработке</th><th>% прибыли</th></tr></thead><tbody>${(monthlyAnalytics.items || []).map((row, idx) => { const moneyIn = boolText(row.money_received) === 'yes'; const rowMarkup = n(row.total_cost) > 0 ? ((n(row.sale_price) - n(row.total_cost)) / n(row.total_cost)) * 100 : 0; return html`<tr className="border-t border-luxe-border"><td className="py-2">${idx + 1}</td><td>${row.model_name || '—'}</td><td className="text-center">${money(row.sale_price)}</td><td className="text-center">${money(row.total_cost)}</td><td className="text-center">${moneyIn ? money(row.profit) : money(0)}</td><td className="text-center">${!moneyIn ? money(row.profit) : money(0)}</td><td className="text-center">${pct(rowMarkup)}</td></tr>`; })}</tbody></table></div><div className="grid md:grid-cols-5 gap-2"><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Продано товаров</p><p className="text-lg font-semibold">${monthlyAnalytics.sold_count || 0}</p></div><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Сумма продаж</p><p className="text-lg font-semibold">${money(monthlyAnalytics.revenue || 0)}</p></div><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Прибыль</p><p className="text-lg font-semibold">${money(monthlyAnalytics.profit || 0)}</p></div><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Прибыль в обработке</p><p className="text-lg font-semibold">${money(monthlyAnalytics.profit_processing || 0)}</p></div><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Прибыль на 1 человека</p><p className="text-lg font-semibold">${money((monthlyAnalytics.profit || 0) / 3)}</p></div></div>${monthlyAnalytics.avg_markup != null ? html`<div className="rounded-xl border border-luxe-border p-3 mt-2"><p className="text-xs text-luxe-muted">Средний % накрутки за месяц</p><p className="text-lg font-semibold">${pct(monthlyAnalytics.avg_markup || 0)}</p></div>` : null}</div>` : html`<div className="mt-3"><label className="text-xs text-luxe-muted">Год<select className="ml-2 rounded-lg border border-luxe-border p-1 bg-white" value=${analyticsYear} onChange=${(e) => setAnalyticsYear(Number(e.target.value))}>${Array.from(new Set(Object.keys(analytics.monthly || {}).map((k) => Number(String(k).slice(0,4))).filter(Boolean))).sort((a,b)=>b-a).map((y) => html`<option value=${y}>${y}</option>`)}</select></label><div className="overflow-auto mt-2"><table className="min-w-[420px] w-full text-sm"><thead><tr className="text-xs text-luxe-muted"><th className="text-left">Месяц</th><th>Продажи</th><th>Прибыль</th><th>В обработке</th><th>Ср. накрутка</th></tr></thead><tbody>${analyticsYearSeries.map((v) => html`<tr className="border-t border-luxe-border"><td className="py-2">${formatMonthRu(v.month)}</td><td className="text-center">${v.sold_count}</td><td className="text-center">${money(v.profit)}</td><td className="text-center">${money(v.profit_processing || 0)}</td><td className="text-center">${pct(v.avg_markup || 0)}</td></tr>`)}</tbody></table></div><div className="mt-3"><${AnalyticsChart} series=${analyticsYearSeries}/></div><div className="grid md:grid-cols-2 lg:grid-cols-4 gap-2 mt-3"><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Общая прибыль за год</p><p className="text-lg font-semibold">${money(analyticsYearTotals.profit)}</p></div><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Средняя прибыль в месяц</p><p className="text-lg font-semibold">${money((analyticsYearTotals.profit || 0) / Math.max(1, analyticsYearSeries.length || 1))}</p></div><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Средний срок продажи</p><p className="text-lg font-semibold">${Math.round(Number(dashboard.avg_sale_days || 0))} дн.</p></div><div className="rounded-xl border border-luxe-border p-3"><p className="text-xs text-luxe-muted">Самый старый товар</p><p className="text-lg font-semibold">${dashboard.oldest_item_number ? `№${dashboard.oldest_item_number} · ${dashboard.oldest_item_days || 0} дн.` : '—'}</p></div></div>${analyticsYearSeries.length > 0 ? html`<div className="rounded-xl border border-luxe-border p-3 mt-2"><p className="text-xs text-luxe-muted">Средний % накрутки за год</p><p className="text-lg font-semibold">${pct(analyticsYearSeries.reduce((a, x) => a + (x.avg_markup || 0), 0) / analyticsYearSeries.length)}</p></div>` : null}</div>`}
      </div></section>`}
    ...
    ${selected && html`<${ItemModal} item=${selected} close=${() => setSelected(null)} save=${saveItem} updateStatus=${updateStatus} updateShipping=${updateShipping} openSale=${(itemNumber) => { setSaleDraftItemNumber(String(itemNumber || '')); setSelected(null); setShowSale(true); }} cancelSale=${cancelSale} deleteItem=${deleteItem} sendToRepair=${(num) => { setSelected(null); setRepairTarget(num); }} isViewer=${isViewer} />`}
    ...
    <nav className="bottom-nav">
      ${(isViewer ? [...BOTTOM_PAGES, ['analytics', 'Аналитика', '📈']] : BOTTOM_PAGES).map(([id, label, icon]) => html`<button onClick=${() => setPage(id)} className=${`nav-btn tap-btn ${id === page ? 'active' : ''}`}>${icon} ${label}</button>`)}
    </nav>

    ${toast && html`<div style=${{ position: 'fixed', bottom: '5.5rem', right: '1rem', background: '#047857', color: 'white', padding: '0.5rem 1rem', borderRadius: '0.75rem', zIndex: 50, fontWeight: 500, boxShadow: '0 4px 12px rgba(0,0,0,0.25)', maxWidth: '90vw' }}>${toast}</div>`}

    ${dialogState && html`<div className="app-dialog-overlay" onClick=${(e) => { if (e.target === e.currentTarget) { dialogState.resolve(null); setDialogState(null); } }} onKeyDown=${(e) => { if (e.key === 'Escape') { dialogState.resolve(null); setDialogState(null); } }}>
      <div className="app-dialog-box" tabIndex="-1">
        <p className="text-center font-medium mb-4">${dialogState.message}</p>
        ${dialogState.type === 'prompt' && html`<input className="w-full rounded-xl border border-luxe-border p-2 mb-4 text-sm" value=${dialogState.inputValue} onInput=${(e) => setDialogState({...dialogState, inputValue: e.target.value})} onKeyDown=${(e) => { if (e.key === 'Enter') { dialogState.resolve(dialogState.inputValue); setDialogState(null); } if (e.key === 'Escape') { dialogState.resolve(null); setDialogState(null); } }}/>`}
        <div className="flex gap-3 justify-center">
          <button className="tap-btn rounded-xl border border-luxe-border px-6 py-2 text-sm" onClick=${() => { dialogState.resolve(null); setDialogState(null); }}>Нет</button>
          <button className="tap-btn rounded-xl bg-luxe-accent text-white px-6 py-2 text-sm" onClick=${() => { dialogState.resolve(dialogState.type === 'prompt' ? dialogState.inputValue : true); setDialogState(null); }}>Да</button>
        </div>
      </div>
    </div>`}
  </div>`;
...
function SaleModal({ close, items, save, initialItemNumber = '' }) {
  ...
  const [f, setF] = useState({ sale_price: '', platform: 'Vinted', money_received: 'no', notes: '', shipping_label_url: '', shipping_status: 'pending', shipping_date: '', tracking_number: '' });
  ...
  useEffect(() => {
    ...
    api('getItemByNumber', { item_number: num })
      .then((res) => {
        ...
        } else if (it.sale_id || String(it.status) === 'sold') {
          setSelectedItem(null);
      ...
      .catch((e) => {
        ...
        if (msg.includes('Unknown action: getItemByNumber')) {
          ...
          if (!local) {
            setSelectedItem(null);
            setLookupError('Товар с таким номером не найден');
          } else if (local.sale_id || String(local.status) === 'sold') {
            setSelectedItem(null);
            setLookupError('Товар уже продан, выберите другой номер');
          } else {
  ...
  const onPickLabel = (e) => {
    ...
    reader.onload = () => {
      ...
      setLabelData(data);
      setF((prev) => ({ ...prev, shipping_label_url: data }));
    };
    reader.readAsDataURL(file);
  };

  return html`<div className="fixed inset-0 bg-overlay pt-16 px-3 pb-3 md:px-8 md:pb-8 z-30 overflow-auto"><form onSubmit=${(e) => { e.preventDefault(); if (!invalid) save({ ...f, sale_price: String(f.sale_price || ''), item_number: selectedItem.item_number, sale_date: new Date().toISOString().slice(0, 10) }); }} className="max-w-xl mx-auto premium-card rounded-2xl p-4 space-y-3"><div className="flex justify-between"><h2 className="font-semibold text-lg">Продажа</h2><button type="button" onClick=${close}>✕</button></div>
    ...
    ${selectedItem ? html`<div className="rounded-xl bg-item-warm p-3 text-sm">Товар: <b>№${selectedItem.item_number} · ${selectedItem.model_name || '—'}</b><br/>Себестоимость: <b>${money(selectedItem.total_cost)}</b> · Статус: <b>${STATUS_META[selectedItem.status]?.label || String(selectedItem.status || '')}</b></div>` : null}
    <div className="grid md:grid-cols-2 gap-2">
      <label className="text-xs">Цена продажи (€)<input type="number" min="0" step="any" className="w-full mt-1 rounded-xl border p-2" value=${String(f.sale_price)} onInput=${(e) => setF((prev) => ({ ...prev, sale_price: e.target.value }))}/></label>
      <label className="text-xs">Платформа<select className="w-full mt-1 rounded-xl border p-2" value=${f.platform} onChange=${(e) => setF((prev) => ({ ...prev, platform: e.target.value }))}>${PLATFORM_OPTIONS.map((p) => html`<option value=${p}>${p}</option>`)}</select></label>
      <label className="text-xs">Деньги зашли<select className="w-full mt-1 rounded-xl border p-2" value=${f.money_received} onChange=${(e) => setF((prev) => ({ ...prev, money_received: e.target.value }))}><option value="no">Нет</option><option value="yes">Да</option></select></label>
    </div>
    <label className="text-xs block">Заметки (опционально)<textarea className="w-full mt-1 rounded-xl border p-2" rows="2" value=${f.notes} onInput=${(e) => setF((prev) => ({ ...prev, notes: e.target.value }))}></textarea></label>
    <div className="rounded-xl border border-luxe-border p-3 bg-white">
      ...
      ${showShipBlock ? html`<div className="mt-2 space-y-2"><p className="text-sm font-medium">Лейбл доставки</p><input type="file" accept="application/pdf,image/*" onChange=${onPickLabel} className="text-sm"/><input className="w-full rounded-xl border p-2 text-sm" value=${f.shipping_label_url} onInput=${(e) => setF((prev) => ({ ...prev, shipping_label_url: e.target.value }))} placeholder="или вставьте ссылку"/>${(labelData || f.shipping_label_url) ? html`<a className="text-blue-700 underline block" href=${labelData || f.shipping_label_url} target="_blank">Открыть лейбл</a>` : null}<label className="text-xs block">Трек-номер (опционально)<input className="w-full mt-1 rounded-xl border p-2" value=${f.tracking_number} onInput=${(e) => setF((prev) => ({ ...prev, tracking_number: e.target.value }))}/></label></div>` : null}
    </div>
...
function ItemModal({ item, close, save, updateStatus, updateShipping, openSale, cancelSale, deleteItem, sendToRepair, isViewer }) {
  if (isViewer) {
    return html`<div className="fixed inset-0 bg-overlay pt-16 px-3 pb-3 md:px-8 md:pb-8 z-60 overflow-auto"><div className="max-w-2xl mx-auto premium-card rounded-2xl p-4 space-y-3 max-h-modal overflow-auto pb-24"><div className="flex justify-between mb-4"><h2 className="font-semibold text-lg">Карточка № ${item.item_number}</h2><button type="button" onClick=${close} className="tap-btn">✕</button></div>
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          <div className="bg-item-warm rounded-xl p-3 space-y-1">
          ...
          ${item.sale_date ? html`<div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 space-y-1">
            ...
            <div className="text-sm"><span className="text-luxe-muted">Прибыль:</span> <b>${boolText(item.money_received) === 'yes' ? money(item.profit) : html`<span className="text-orange-600">в ожидании</span>`}</b></div>
  ...
  <div className="grid md:grid-cols-2 gap-2">
    ...
    <label className="text-xs inline-flex items-center gap-2"><input type="checkbox" checked=${boolText(f.listed_vinted) === 'yes'} onChange=${(e) => { const nextV = e.target.checked ? 'yes' : 'no'; const either = nextV === 'yes' || boolText(f.listed_vestiaire) === 'yes'; setF({ ...f, listed_vinted: nextV, arrived_from_japan: either ? 'yes' : f.arrived_from_japan, japan_arrival_date: either ? (f.japan_arrival_date || new Date().toISOString().slice(0,10)) : f.japan_arrival_date }); }}/>Выставлено на Vinted</label>
    <label className="text-xs inline-flex items-center gap-2"><input type="checkbox" checked=${boolText(f.listed_vestiaire) === 'yes'} onChange=${(e) => { const nextV = e.target.checked ? 'yes' : 'no'; const either = nextV === 'yes' || boolText(f.listed_vinted) === 'yes'; setF({ ...f, listed_vestiaire: nextV, arrived_from_japan: either ? 'yes' : f.arrived_from_japan, japan_arrival_date: either ? (f.japan_arrival_date || new Date().toISOString().slice(0,10)) : f.japan_arrival_date }); }}/>Выставлено на Vestiaire</label>
  ...
  return html`
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-luxe-bg via-luxe-bg to-[#ece6db] p-4">
      <form className="premium-card rounded-2xl p-8 w-full max-w-md space-y-6 shadow-xl" onSubmit=${async (e) => { e.preventDefault(); setError(''); try { const r = await onLogin(identity, password, workspaceId); if (r?.require_workspace_choice) { setWorkspaces(r.workspaces || []); setWorkspaceId(r.workspaces?.[0]?.id || ''); } } catch (err) { setError(String(err.message || 'Ошибка входа')); } }}>
        ...
        <div className="space-y-4">
          <input className="w-full rounded-xl border border-luxe-border p-3 text-lg text-center focus:outline-none focus:ring-2 focus:ring-luxe-accent" placeholder="Логин" value=${identity} onInput=${(e)=>setIdentity(e.target.value)} />
          <div className="relative">
            <input className="w-full rounded-xl border border-luxe-border p-3 pr-12 text-lg text-center focus:outline-none focus:ring-2 focus:ring-luxe-accent" type=${showPassword ? 'text' : 'password'} placeholder="Пароль" value=${password} onInput=${(e)=>setPassword(e.target.value)} />
            <button type="button" style=${{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.1rem', color: '#7b6755', padding: '0.25rem' }} onClick=${() => setShowPassword(!showPassword)} title=${showPassword ? 'Скрыть пароль' : 'Показать пароль'}>${showPassword ? '🙈' : '👁️'}</button>
          </div>
...
function RootApp() {
  ...
  return html`<${CrmApp} onLogout=${logout} session=${session} onSessionUpdate=${updateSessionUser} />`;
}
